...
## Summary
Collection of Lynx IA coding agents, skills, and actions for the Agent Smith framework — provides a multi-agent coding team with coordinator, planner, reviewer, and specialist agents (including Go-specific agents).

## Dependencies
- **Internal**: `@agent-smith/core` (^0.0.15) — runtime for agent execution, feature discovery, and tool integration
- **External**: `@boxlite-ai/boxlite` (^0.9.7) — runtime dependency for agent execution
...
## Entry Point
- `dist/agents/*.yml` — 22 YAML agent definitions loaded as features by @agent-smith/core
...
| `dist/agents/lxgo.yml` | Go-language coordinator agent (qwen35b) |
| `dist/agents/lxgo-team.yml` | Go team coordinator with lxgo-review, lxgo-assistant, lx-search (qwen35b) |
| `dist/agents/lxgo-assistant.yml` | Go assistant agent (qwen35b) |
| `dist/agents/lxgo-review.yml` | Go adversarial code reviewer (qwen35b) |
| `dist/agents/lxgots.yml` | Go test agent (qwen35b) |
...
- **Go-Specific Agents**: Recent update (commit 64c3d34) added Go-language agents (`lxgo.yml`, `lxgo-team.yml`, `lxgo-assistant.yml`, `lxgo-review.yml`, `lxgots.yml`) with enhanced shell/npm tooling and improved sanitization
...
## Related
- See `/workspace/agent-smith/packages/core` — runtime that discovers and executes lynx-coder agents
- See `/workspace/agent-smith-plugins` — similar plugin structure with YAML agents and skills
- See `/workspace/.agents/documentation/project-nav.md` — navigation map including lynx-coder agents
